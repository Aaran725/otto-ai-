...
{
  "intent": "undervalued" | "momentum" | "best" | "quality" | "avoid" | "contrarian" | null,
  "theme": { "label": string, "keywords": string[] } | null,
  "minMarketCapMillions": number | null,
  "seedTickers": string[],
...
Rules:
- "intent" is the ranking style: "undervalued" (cheap/value), "momentum" (hot/trending stocks with strong expected forward upside — this covers common finance slang like "rocket stock(s)", "moonshot", "breakout", "hot stock", "surging", "stocks that might go boom/high upside" even when that slang isn't a literal industry), "quality" (safe/stable/defensive), "avoid" (red flags/risky), "contrarian" (specifically comparing Otto's own forecast against Wall Street analyst consensus — phrases like "where does Otto disagree with Wall Street", "contrarian picks", "Otto vs analysts/the Street" — do NOT use this for a generic "best pick" or "undervalued" ask that merely happens to mention analysts), "best" (general "good stock" ask, or no clearer signal). Use null only if this message isn't a stock-screening request at all.
